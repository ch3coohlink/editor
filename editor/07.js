...
$.fakeidb = (name = "default", store = "default") => {
  let $ = { name, store }; with ($) {
    $.o = {}, $.cmp = t => (v, i) =>
      v.startsWith(t) ? 0 : v.localeCompare(t)
    $.get = async k => o[k], $.set = async (k, v) => o[k] = v
    $.del = async k => delete o, $.clr = async () => o = {}
    $.key = async () => Object.keys(o), $.val = async () => Object.values(o)
    $.getpath = async t => {
      t = t[t.length - 1] === "/" ? t : t + "/"
      const a = (await key()).sort((a, b) => a.localeCompare(b))
      const f = cmp(t), s = bsearch(a, f), l = t.length
      return s < 0 ? [] : a.slice(bsleft(a, f), bsright(a, f) + 1)
        .map(k => [k.slice(l), o[k]])
    }
    $.logidb = (a = [''], i = 0) => {
...
$.graphdb = ($ = eventnode({})) => {
  with ($) {
    ...
    $.addnode = async (id = uuid(), g = current_graph) => {
      await Promise.all([idb.set(`$graph/${g}/id/${id}`, true),
    ...
    $.delnode = async (id, g = current_graph) => {
      const [to, from, keys] = await Promise.all([
        ...
        idb.getpath(`$graph/${g}/id/${id}/`)
      ]); await Promise.all([
        ...to.map(([k]) => deledge(id, k)),
        ...from.map(([k]) => deledge(k, id)),
      ]); await Promise.all([
        ...keys.map(([k]) => delprop(a, k, g)),
        idb.del(`$graph/${g}/id/${id}`, true),
    ...
    $.addedge = async (a, b, n, g = current_graph) => {
      ...
      if (n) p.push(idb.set(`$graph/${g}/prop/${a}/${n}`, b),
        idb.set(`$graph/${g}/revprop/${n}/${a}`, true),
        idb.set(`$graph/${g}/edgename/${a}/${b}`, n))
    ...
    $.deledge = async (a, b, g = current_graph) => {
      ...
      p.push(idb.del(`$graph/${g}/to/${a}/${b}`),
        ...
        idb.set(`$graph/${g}/numberfrom/${b}`, nfr - 1))
      if (n) p.push(...delprop(a, n, g),
        idb.del(`$graph/${g}/edgename/${a}/${b}`))
      await Promise.all(p)
    }
    $.deledgebyname = async (a, n, g = current_graph) => {
      const b = await getprop(a, n, g)
    ...
    $.nameedge = async (a, b, n, g = current_graph) => {
      ...
      await Promise.all([setprop(a, n, b, g),
      idb.set(`$graph/${g}/edgename/${a}/${b}`, n)])
    }
    $.unnameedge = async (a, b, g = current_graph) => {
      ...
      await Promise.all([delprop(a, n, g),
      idb.set(`$graph/${g}/edgename/${a}/${b}`)])
    }
    ...
    $.renameedge = async (a, n, nn, g = current_graph) => {
      const b = await getprop(a, n, g)
      if (!b) { throw Error(`edge not exist: ${a}:${n}`) }
      await Promise.all([delprop(a, n, g), setprop(a, nn, b, g),
      idb.set(`$graph/${g}/edgename/${a}/${b}`, nn)])
    }
    $.getprop = (a, k, g) => idb.get(`$graph/${g}/prop/${a}/${k}`)
    $.delprop = (a, k, g = current_graph) => Promise.all([
      idb.del(`$graph/${g}/prop/${a}/${k}`),
      idb.del(`$graph/${g}/revprop/${k}/${a}`)])
    $.setprop = (a, k, v = true, g = current_graph) => Promise.all([
      idb.set(`$graph/${g}/prop/${a}/${k}`, v),
      idb.set(`$graph/${g}/revprop/${k}/${a}`, true)])
    $.getallnode = (tag, g = current_graph) => (tag
      ? idb.getpath(`$graph/${g}/revprop/${tag}`)
      : idb.getpath(`$graph/${g}/id`)).then(a => a.map(([k]) => k))
    $.getalledge = (ns, g = current_graph) =>
      Promise.all(ns.map(id => idb.getpath(`$graph/${g}/to/${id}`)
...
$.gengraph = async () => {
  ...
  for (let i = 0; i < l; i++) {
    ...
    ids.push(id)
    await g.setprop(id, '__visible__', true)
  }
  for (let i = 0; i < l; i++) {
    let r = floor(abs(gaussian(0, 2)) * 1) + (rd() > 0.5 ? 1 : 0)
    const a = ids[i], s = [...ids]
    for (let j = 0; j < r && s.length > 0; j++) {
      const b = s.splice(floor(rd(s.length)), 1)[0]
      const n = 'NAME_' + ids.indexOf(b)
      if (rd() > 0.0) { await g.addedge(a, b, n) }
...
$.grapheditor = ($ = { g: graph(), gdb: graphdb(), idb }) => {
  with ($) {
    const a = ('delnode addedge deledge deledgebyname ' +
      'nameedge unnameedge unnameedgebyname renameedge').split(' ')
    for (const n of a) $[n] =
      async (...a) => (await gdb[n](...a), g[n](...a))
    $.addnode = async (id = uuid()) => (await gdb.addnode(id), g.addnode(id))
    $.sync = async () => {
      const ns = await gdb.getallnode('__visible__')
      const es = await gdb.getalledge(ns)
      for (const n of ns) { g.namenode(g.g[g.addnode(n)], n.slice(0, 8)) }
      for (const e of es) { g.addedge(...e) }
    }
    $.frame = () => { g.frame() }
