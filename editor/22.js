  ...
  $.sandboxtab = (vcs, target, $ = eventnode(dom())) => {
    $.vcs = vcs; $.target = target; with ($) {
      ...
      $.exec = async () => {
        ...
        const __read__ = (v, b, fst) => (p, opt) =>
          read(v, solvepath(b, p), fst, opt, !(opt?.raw))
        const __write__ = (v, b, fst) => (p, t, force) =>
          write(v, solvepath(b, p), t, fst, force)
        const __dirname = filepath.split('/').slice(0, -1).join('/')
        env.__sandbox__ = { read, write }
        env.__read__ = __read__
...
const localsave = async dir => {
  ...
  $.repo = sp.get('repo'); if (userland) {
    ...
    if (typeof repo !== 'string') { repo = '9954a7c53d08fcca' }
    await httpload(repo)
  }
}; let userland
