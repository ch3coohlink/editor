...
fn randomvector(id: vec2f) -> vec2f {
  ...
  // return sin(sin(g) * 143758. + uniforms.time);
}
...
fn rendering(id: vec2f, resolution: vec2f) -> vec3f {
  let t = uniforms.time; var uv = (id.xy + 0.5
     - resolution.xy * 0.5) / resolution.y; uv.y = -uv.y;
  // uv *= 10.0; let ouv = uv;
  uv *= t; let ouv = uv;
  let uvid = floor(uv); uv = fract(uv);
  let bl = randomvector(uvid + vec2(0)); let br = randomvector(uvid + vec2(1, 0));
  let tl = randomvector(uvid + vec2(0, 1)); let tr = randomvector(uvid + vec2(1));
  ...
  // uv = cubic(uv);
  uv = quintic(uv);
  let d = pow((ouv.x * ouv.x + ouv.y * ouv.y), 1) / t;
  let p = mix(mix(dbl, dbr, uv.x), mix(dtl, dtr, uv.x), uv.y) - d;
  // return vec3f(abs(p));
  // return vec3f(p);
  // return vec3f(0.5 * (p + 1));
  if(p < -1) { return vec3(0); }
  // these lines are not identical ???
  // else if(p < 1 - 2 / 4 * 3) { return vec3(1, 0, 1); }
  // else if(p < 1 - 2 / 4 * 2) { return vec3(1, 1, 0); }
  // else if(p < 1 - 2 / 4 * 1) { return vec3(0, 1, 1); }-
  else if(p < -0.5) { return vec3(1, 0, 1); }
  else if(p < 0) { return vec3(1, 1, 0); }
  else if(p < 0.5) { return vec3(0, 1, 1); }
  else { return vec3(1); }
  return vec3f(uv, 0);
}
