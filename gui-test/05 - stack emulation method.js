...
fn rayoctree(vro: vec3f, vrd: vec3f, clr: ptr<function, vec3f>) -> f32 {
  ...
  stack[si].pos = vec3(0);
  stack[si].t0 = t0; stack[si].t1 = t1; stack[si].state = 0;
  loop {
    let cs = stack[si].state;
    if(cs == 0) {
      stack[si].tm = 0.5 * (stack[si].t0 + stack[si].t1);
      stack[si].i = firstnode(stack[si].t0, stack[si].tm, stack[si].t1);
      stack[si].state = 1;
    } else if(cs == 1) {
      let mask = vec3<bool>(bool((stack[si].i >> 2) & 1),
        bool((stack[si].i >> 1) & 1), bool(stack[si].i & 1));
      let nt0 = select(stack[si].t0, stack[si].tm, mask);
      let nt1 = select(stack[si].tm, stack[si].t1, mask);
      stack[si].mask = mask; stack[si].state = 2;

      let size = 1 / f32(1 << u32(si + 1));
      let realindex = stack[si].i ^ mirrormask;
      let realmask = vec3<bool>(bool((realindex >> 2) & 1),
        bool((realindex >> 1) & 1), bool(realindex & 1));
      let npos = stack[si].pos + select(vec3(0), vec3(size), realmask);
      let v = rnd(vec4f(npos, size));
      if(v < 0.4) { continue; } // empty

      step++; si++; stack[si].pos = npos;
      stack[si].t0 = nt0; stack[si].t1 = nt1; stack[si].state = 0;
    } else if(cs == 2) {
      let v = select(stack[si].tm, stack[si].t1, stack[si].mask);
      let i = nextnode_lut[stack[si].i][minindex(v)]; stack[si].i = i;
      if(i < 8) { stack[si].state = 1; } else { si--; }
